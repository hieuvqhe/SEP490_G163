...
const STATUS_MAINTENANCE = ["disabled", "inactive", "unavailable", "maintenance"];
...
const SeatLayout = () => {
  const seatLayoutContext = usePartnerHomeStore((state) => state.seatLayoutContext);
  ...
  const [selectedSeatType, setSelectedSeatType] = useState<SeatTypeOption | null>(null);
  const [seatTypeActionValue, setSeatTypeActionValue] = useState<string | undefined>(
    undefined
  );
  ...
  const templateSeatType = useMemo(
    () => seatTypeOptions.find((type) => type.name === TEMPLATE_SEAT_TYPE_NAME) ?? null,
    [seatTypeOptions]
  );

  const blockedSeatType = useMemo(
    () => seatTypeOptions.find((type) => type.name === BLOCKED_SEAT_TYPE_NAME) ?? null,
    [seatTypeOptions]
  );
  ...
  useEffect(() => {
    if (!seatLayoutQuery.data?.result || !seatLayoutQuery.data.result.seatMap?.hasLayout) {
      return;
    }
  ...
  const handleGenSeat = (e: React.FormEvent) => {
    ...
    const fallbackSeatType = selectedSeatType ?? templateSeatType ?? seatTypeOptions[0] ?? null;
  ...
  const handleUpdateSelected = ({
    ...
    if (selectedSeats.length === 0) {
      showToast("Vui lòng chọn ít nhất một ghế để cập nhật.", undefined, "info");
      return false;
    }
    ...
    setSeats((prevSeats) =>
      prevSeats.map((seat) => {
        ...
        const appliedColor = seatType ? seatType.color : color ?? seat.colorCode;
  ...
  const hasExistingLayout = seatLayoutQuery.data?.result?.seatMap?.hasLayout ?? false;
  const isSaving =
    createSeatLayoutMutation.isPending || updateSeatLayoutMutation.isPending;

  const handleSaveSeatLayout = () => {
    if (!screenId) {
      showToast("Vui lòng chọn phòng chiếu trước khi lưu sơ đồ.", undefined, "error");
      return;
    }
    ...
    if (!screenId) {
      return (
        <p className="text-xs text-slate-500">
          Bạn có thể tạo sơ đồ mẫu để tham khảo hoặc chọn một phòng chiếu từ tab "Phòng chiếu".
        </p>
      );
    }

    if (seatLayoutQuery.isLoading) {
      return <p className="text-xs text-slate-400">Đang tải sơ đồ ghế từ hệ thống...</p>;
    }
    ...
    if (seatLayoutQuery.data?.result?.seatMap?.hasLayout) {
      return (
        <p className="text-xs text-emerald-300">
          Đã tải sơ đồ ghế thực tế (cập nhật: {new Date(seatLayoutQuery.data.result.seatMap.updatedAt).toLocaleString("vi-VN")} ).
        </p>
      );
    }

    return <p className="text-xs text-slate-400">Phòng chưa có sơ đồ ghế trên hệ thống. Bạn có thể tự tạo mẫu bên dưới.</p>;
  };
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-white">Thiết kế sơ đồ ghế</h2>
          {seatLayoutContext.screenName ? (
            <p className="text-sm text-slate-300">
              Đang xem phòng: <span className="font-medium text-orange-300">{seatLayoutContext.screenName}</span>
              {seatLayoutContext.cinemaName ? ` • ${seatLayoutContext.cinemaName}` : ""}
            </p>
          ) : (
            <p className="text-sm text-slate-400">
              Bạn có thể tự tạo sơ đồ ghế mẫu để tham khảo (chưa kết nối dữ liệu).
            </p>
          )}
          {renderApiStatus()}
        </div>
        ...
        <div className="flex-1 space-y-8">
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
              <form onSubmit={handleGenSeat} className="flex-1 space-y-4">
                <div className="space-y-2">
                  <h3 className="text-lg font-semibold text-white">Tạo sơ đồ ghế</h3>
                  <p className="text-sm text-zinc-300/70">
                    Điều chỉnh số hàng, số cột và loại ghế mặc định trước khi tạo sơ đồ.
                  </p>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-[2fr_1fr_1fr_auto] md:items-end">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={selectedSeatType ? String(selectedSeatType.id) : undefined}
                      onValueChange={(value) => {
                        const type = seatTypeOptions.find((item) => item.id === Number(value)) ?? null;
                        setSelectedSeatType(type);
                      }}
                    >
                      <SelectTrigger className="bg-zinc-800">
                        <SelectValue placeholder={isLoading ? "Đang tải..." : "Chọn loại ghế"} />
                      </SelectTrigger>
                      <SelectContent className="bg-zinc-800 text-zinc-100">
                        <SelectGroup>
                          <SelectLabel>Các kiểu ghế</SelectLabel>
                          {seatTypeOptions.map((seatType) => (
                            <SelectItem key={seatType.id} value={String(seatType.id)}>
                              {seatType.name}
                            </SelectItem>
                          ))}
                        </SelectGroup>
                ...
                <div className="w-full max-w-[220px] space-y-3 rounded-lg border border-zinc-800 bg-zinc-900/80 p-5 shadow">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-zinc-100">{selectedSeatType.color}</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-sm text-zinc-400">Tên loại ghế</p>
                    <p className="font-semibold text-zinc-100">{selectedSeatType.name}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="rounded-xl flex flex-col items-center border border-slate-800/60 bg-zinc-900/60 p-6 shadow-lg">
            <div className="mb-6 flex flex-col items-center gap-2 text-center text-slate-300">
              ...
              <p className="text-xs uppercase tracking-widest text-slate-500">Màn hình</p>
            </div>

            {seats.length === 0 ? (
              <div className="flex min-h-[320px] items-center justify-center rounded-lg border border-dashed border-slate-700/50 bg-slate-900/40 text-sm text-slate-400">
                ...
                >
                  <div className="flex w-full max-w-5xl flex-col items-center gap-4">
                        ...
                        return (
                          <div
                            ...
                            className={`cursor-pointer text-center ${
                              isColumnSelected ? "text-yellow-400" : "text-slate-400"
                            }`}
                          >
                            {column}
                          </div>
                    ...
                    {rowLabels.map((rowLabel) => {
                      ...
                      return (
                        <div key={rowLabel} className="flex w-full items-center gap-3">
                          <div
                            onClick={() => toggleRowSelect(rowLabel)}
                            className={`w-10 cursor-pointer text-center text-sm font-semibold ${
                              isRowSelected ? "text-yellow-400" : "text-slate-400"
                            }`}
                          >
                            {rowLabel}
                          </div>
                          ...
                          >
                            {columns.map((column) => {
                              ...
                              const label = isMaintenance ? "Z0" : `${seat.row}${++seatOrder}`;
          ...
          <div className="rounded-xl border border-zinc-700 bg-zinc-900/70 p-4 shadow-md">
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {seatTypeOptions.length > 0 ? (
                seatTypeOptions.map((seatType) => (
                  <div key={seatType.id} className="flex items-center gap-3 rounded-lg border border-zinc-800/60 bg-zinc-900/40 p-3">
                    <span
                      className="inline-flex h-4 w-4 shrink-0 rounded border border-white/20"
                      style={{ backgroundColor: seatType.color }}
                    />
                    <div className="space-y-0.5">
                      <p className="text-sm font-medium text-zinc-100">{seatType.name}</p>
                      {seatType.code && (
                ...
                <p className="text-sm text-zinc-500">Chưa có dữ liệu loại ghế.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <DialogContent className="max-w-3xl border border-zinc-800 bg-zinc-900 text-zinc-100">
          ...
          <div className="space-y-4">
            <div className="max-h-96 overflow-auto rounded-lg border border-zinc-800 bg-zinc-950/60 p-4 text-sm font-mono">
              {seatLayoutJson ? (
                <pre className="whitespace-pre-wrap break-words text-zinc-100">{seatLayoutJson}</pre>
              ) : (
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {copyStatus === "copied" && (
                <span className="text-xs text-emerald-400">Đã sao chép vào clipboard.</span>
              )}
              {copyStatus === "error" && (
                <span className="text-xs text-red-400">Sao chép không thành công. Vui lòng thử lại.</span>
              )}
            </div>
          </div>
        </DialogContent>
