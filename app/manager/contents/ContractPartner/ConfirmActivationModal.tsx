...
const ConfirmActivationModal = ({
  ...
  const isStaffManager = role === "ManagerStaff";
      ...
      >
        <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
          <h2 className="text-lg font-semibold">
            {isStaffManager ? "Kí xác nhận hợp đồng" : "Kích hoạt hợp đồng"}
          </h2>
          <button
            onClick={onCancel}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 space-y-6">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-sm text-gray-200">
              <p className="font-medium text-white">
                {isStaffManager
                  ? `Bạn có chắc muốn kí xác nhận hợp đồng #${contractId}?`
                  : `Bạn có chắc muốn kích hoạt hợp đồng #${contractId}?`}
              </p>
              <p>
                {isStaffManager
                  ? "Hành động này sẽ khóa hợp đồng và gửi hợp đồng đến cho Quản lí. Vui lòng đảm bảo mọi thông tin đã chính xác trước khi xác nhận."
                  : "Hành động này sẽ khóa hợp đồng và không thể hoàn tác. Vui lòng đảm bảo mọi thông tin đã chính xác trước khi xác nhận."}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <label className="block space-y-2 text-sm">
              ...
              />
              {signatureError ? (
                <p className="text-xs text-red-400">{signatureError}</p>
              ) : null}
            </label>
