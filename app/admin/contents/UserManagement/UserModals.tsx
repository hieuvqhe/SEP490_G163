...
export const UserDetailModal = ({ user, onClose }: UserDetailModalProps) => {
  ...
  const { data: userDetail, isLoading, error } = useGetUserById(user.id, accessToken || undefined);
      ...
      >
        ...
        >
          <div className="flex justify-center items-center py-12">
            ...
            <span className="ml-3 text-white font-body">Đang tải thông tin người dùng...</span>
          </div>
        </motion.div>
      </motion.div>
    );
        ...
        >
          ...
          {detailData?.avataUrl && (
            ...
            >
              ...
              />
              <div className="w-24 h-24 rounded-full bg-blue-500/20 border-4 border-blue-500/50 flex items-center justify-center absolute inset-0 opacity-0 transition-opacity duration-200" style={{ display: 'none' }}>
                <span className="text-2xl font-bold text-blue-400">
                  {(detailData.fullname || detailData.username || 'U').charAt(0).toUpperCase()}
                </span>
              </div>
            </motion.div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-gray-400 text-sm font-body">Họ tên đầy đủ</p>
                  <p className="text-white font-body break-words">{detailData?.fullname || 'Chưa cập nhật'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-white font-body break-words">{detailData?.email}</p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-white font-body">{detailData?.phone || 'Chưa cập nhật'}</p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-white font-body break-words">{detailData?.username}</p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-white font-body">
                    {detailData?.createdAt ? new Date(detailData.createdAt).toLocaleDateString('vi-VN') : 'N/A'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="text-gray-400 text-sm font-body mb-2">Loại người dùng</p>
                <span className={`px-3 py-1 rounded-full text-sm font-medium font-body ${
                  detailData?.userType === 'admin' ? 'bg-red-500/20 text-red-400 border border-red-500/30' :
                  detailData?.userType === 'manager' ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' :
                  detailData?.userType === 'partner' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' :
                  detailData?.userType === 'employee' ? 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30' :
              ...
              <div>
                <p className="text-gray-400 text-sm font-body mb-2">Email xác thực</p>
                <span className={`px-3 py-1 rounded-full text-sm font-medium font-body ${
              ...
              <div>
                <p className="text-gray-400 text-sm font-body mb-2">Trạng thái hoạt động</p>
                <span className={`px-3 py-1 rounded-full text-sm font-medium font-body ${
                  detailData?.isActive 
                    ? 'bg-green-500/20 text-green-400 border border-green-500/30' 
                    : 'bg-red-500/20 text-red-400 border border-red-500/30'
              ...
              <div>
                <p className="text-gray-400 text-sm font-body mb-2">Trạng thái cấm</p>
                <span className={`px-3 py-1 rounded-full text-sm font-medium font-body ${
                  detailData?.isBanned 
                    ? 'bg-red-500/20 text-red-400 border border-red-500/30' 
                    : 'bg-green-500/20 text-green-400 border border-green-500/30'
          ...
          {(detailData?.bannedAt || detailData?.unbannedAt || detailData?.deactivatedAt) && (
            <div className="mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {detailData.bannedAt && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                    <p className="text-red-400 text-sm font-body mb-1">Ngày cấm</p>
                ...
                {detailData.unbannedAt && (
                  <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                    <p className="text-green-400 text-sm font-body mb-1">Ngày bỏ cấm</p>
                    <p className="text-white font-body text-sm">
                ...
                {detailData.deactivatedAt && (
                  <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3">
                    <p className="text-yellow-400 text-sm font-body mb-1">Ngày vô hiệu hóa</p>
                    <p className="text-white font-body text-sm">
                      {new Date(detailData.deactivatedAt).toLocaleString('vi-VN')}
                    </p>
                  </div>
                )}
              </div>
          ...
          {detailData?.avataUrl && (
            <div className="mt-6">
              ...
              <div className="bg-slate-700/50 rounded-lg p-4">
                <p className="text-gray-400 text-sm font-body mb-2">URL Avatar</p>
                <a 
      ...
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
        <div className="bg-gray-800 rounded-lg p-6 max-w-2xl w-full mx-4">
          <div className="flex justify-center items-center py-8">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
            <span className="ml-2 text-white">Đang tải thông tin người dùng...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
        <div className="bg-gray-800 rounded-lg p-6 max-w-2xl w-full mx-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-bold text-white font-heading">Lỗi tải dữ liệu</h3>
      ...
      <div className="bg-gray-800 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1 font-body">Loại người dùng</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1 font-body">Họ tên đầy đủ</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1 font-body">Tên người dùng</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1 font-body">URL Avatar</label>
              <input
                type="url"
                value={formData.avataUrl}
                onChange={(e) => setFormData({ ...formData, avataUrl: e.target.value })}
                className="w-full px-3 py-2 bg-gray-700 text-white rounded border border-gray-600 focus:outline-none focus:border-blue-500 font-body"
              />
            </div>
          </div>
...
export const UnbanConfirmModal = ({ user, onClose, onConfirm }: UnbanConfirmModalProps) => (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-bold text-white font-heading">Xác nhận bỏ cấm</h3>
        <button onClick={onClose} className="text-gray-400 hover:text-white">
          <X size={24} />
        </button>
      </div>
...
export const BanConfirmModal = ({ user, onClose, onConfirm }: BanConfirmModalProps) => (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-bold text-white font-heading">Xác nhận cấm người dùng</h3>
        <button onClick={onClose} className="text-gray-400 hover:text-white">
          <X size={24} />
        </button>
      </div>
